...
class Mod_action extends CI_Model
{
    ...
    {
        $this->db->select('id_pic, pic_name');
        ...
        foreach ($datapic as $pic) {
            $data[] = array("id" => $pic['pic_name'], "text" => $pic['pic_name']);
        }
        return $data;
    }
    ...
    function get_sectdua( $searchTerm = "")
    {
        $this->db->select('id_sect, section_name');
        $this->db->where("section_name like '%" . $searchTerm . "%' ");
        $this->db->order_by('section_name', 'asc');
        ...
        $data = array();
        foreach ($datasect as $sect) {
            $data[] = array("id" => $sect['section_name'] , "text" => $sect['section_name']);
        }
        return $data;
    }
